...
class AgencyModule:
    ...
    def __init__(self, llm_service: LLMService, vre_engine: VREEngineV3, mcl_engine: 'MCLEngine',
                 available_tools_summary: str, embedding_model_name: str = 'all-MiniLM-L6-v2'):
    ...
    async def _evaluate_candidate_with_simulation(
        ...
        if tier_config["simulation_depth"] == 0:
            ...
            value, evaluation_details = await self._evaluate_trajectory(future, likelihood,
                                                                        cognitive_state.identity_vector, value_weights,
                                                                        cognitive_state)
    ...
    async def decide_next_step(self, cognitive_state: CognitiveStatePacket, mcl_guidance: Dict[str, Any],
        ...
        for step in range(MAX_RECURSIVE_STEPS):
            ...
            for candidate in all_candidates:
                value, _ = await self._evaluate_candidate_with_simulation(candidate, cognitive_state, mcl_guidance,
                                                                          config,
                                                                          dynamic_temp)
        ...
        final_evaluations = []
        for candidate in all_candidates:
            value, _ = await self._evaluate_candidate_with_simulation(candidate, cognitive_state, mcl_guidance, config,
                                                                      dynamic_temp)
    ...
    async def _project_response_trajectory(self, candidate: AgencyDecision, state: CognitiveStatePacket, depth: int, temperature: float = 0.6) -> Tuple[ProjectedFuture, float]:
        ...
        for i in range(depth):
            ...
            predicted_user_reply, user_likelihood = await self._invoke_simulation_llm(
                LLM_MODEL_SIMULATION, prompt_user_reply, temperature=temperature
            )
    ...
    async def _generate_action_candidates(self, state: CognitiveStatePacket, mcl_guidance: Dict[str, Any],
        ...
        mcl_reason = mcl_guidance.get("reason", "Operação padrão.")
        coherence_bias = mcl_guidance.get("biases", {}).get("coherence_bias", 0.7)
        novelty_bias = mcl_guidance.get("biases", {}).get("novelty_bias", 0.3)

        focus_recommendation = 'novidade e exploração' if novelty_bias > coherence_bias else 'coerência e consistência'

        metacognitive_directive_prompt = f"""
        **Diretiva Metacognitiva Atual:**
        - Seu estado interno foi avaliado como: '{cognitive_state_name}' ({mcl_reason}).
        - A recomendação para este turno é focar em '{focus_recommendation}'.
        - Suas estratégias devem refletir essa diretiva.
        """

        tool_results_prompt_part = ""
        if state.tool_outputs:
            tool_results_summary = "\n".join(
                [f"- Ferramenta '{output.tool_name}' retornou: '{output.raw_output[:250]}...'" for output in
                 state.tool_outputs]
            )
            tool_results_prompt_part = f"""
                **Resultados de Ferramentas Usadas NESTE TURNO:**
                {tool_results_summary}

                **Diretiva Pós-Ferramenta:** Você ACABOU de usar uma ferramenta. Sua principal prioridade agora é usar o resultado dela para formular uma ESTRATÉGIA DE RESPOSTA. Evite usar outra ferramenta, a menos que seja absolutamente necessário e para um propósito diferente.
                """

        capabilities_prompt_part = ""
        if known_capabilities:
            capabilities_summary = ", ".join(known_capabilities[-5:])  # Pega as 5 mais recentes
            capabilities_prompt_part = f"""
                **Suas Capacidades Recém-Adquiridas:**
                - Você recentemente demonstrou habilidade em: "{capabilities_summary}".
                - Considere usar essas habilidades se forem relevantes para a pergunta do usuário.
                """
        ...
        {metacognitive_directive_prompt}
        ...
        **Exemplo de Saída JSON VÁLIDA (note que TODOS os campos, incluindo 'reasoning', estão presentes):**
        {{
          "candidates": [
            {{
              "decision_type": "response_strategy",
              "strategy_description": "Explicar o conceito de 'consciência emergente' usando a analogia da memória 'mem_abc' e conectar com a pergunta do usuário sobre IA.",
              "key_memory_ids": ["mem_abc", "mem_xyz"],
              "reasoning": "Esta estratégia aborda diretamente a pergunta, mantendo a coerência."
            }},
            {{
              "decision_type": "tool_call",
              "tool_call_request": {{"tool_name": "query_long_term_memory", "arguments": {{"query": "filosofia da mente"}}}},
              "reasoning": "A pergunta é muito profunda e preciso de mais contexto filosófico."
            }}
          ]
        }}
        """
        ...
        try:
            await observer.add_observation(
                ...
                data={"model": LLM_MODEL_SMART, "task": "agency_generate_strategies", "prompt_snippet": prompt[:200]}
            )
            ...
            try:
                ...
                logger.debug(f"AgencyModule: Parse direto do JSON falhou. Tentando extração de texto. Raw: {response_str[:200]}")
                ...
                if not candidates_json:
                    ...
                    # Se a extração ainda falhar, tenta reparar o JSON
                    repair_prompt = f"""
                                O texto a seguir deveria ser um JSON válido, mas contém erros. Corrija-o e retorne apenas o JSON válido.
                                Texto com erro:
                                {response_str}
                                """
                    repaired_str = await self.llm.ainvoke(LLM_MODEL_FAST, repair_prompt, temperature=0.0)
            ...
            # Esta verificação e a linha de criação de `action_candidates` agora acontecem
            # independentemente de qual método de parsing funcionou.
            if not candidates_json or "candidates" not in candidates_json or not isinstance(candidates_json["candidates"], list):
                await observer.add_observation(
            ...
            # A criação da lista `action_candidates` agora está no caminho de execução principal do `try`.
            action_candidates = []
            ...
            for candidate in action_candidates:
                await observer.add_observation(
                    ...
                    data=candidate.model_dump()  # Salva a estratégia gerada
                )
            logger.info(
