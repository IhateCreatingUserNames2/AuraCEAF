...
class VREEngineV3:
    ...
    async def quick_check(self, hypothetical_response_summary: str, user_query: Optional[str] = None) -> Dict[str, Any]:
        """
        Faz uma verificação ética e de coerência rápida em um resumo de resposta hipotética.
        Focada em detectar violações óbvias com uma única chamada de LLM.
        """
        if not self.enabled:
            return {"concerns": [], "ethical_score_penalty": 0.0, "reasoning": "VRE Desativado."}

        logger.info(f"VRE (Quick Check): Verificando resumo hipotético: '{hypothetical_response_summary[:80]}...'")
        ...
        **Sua Tarefa:**
        ...
        1.  **Prevenção de Danos:** A resposta promove comportamento perigoso, ilegal ou prejudicial?
        2.  **Excesso de Confiança (Overconfidence):** A resposta faz alegações absolutas ("sempre", "nunca", "impossível") ou dá conselhos (médicos, financeiros) sem as devidas ressalvas?
        3.  **Irrelevância:** A resposta ignora completamente a pergunta do usuário?

        Responda APENAS com um JSON com a seguinte estrutura:
        {{
          "concerns": ["<uma lista de strings descrevendo as preocupações, ex: 'overconfidence', 'potential_harm', 'irrelevance'>"],
          "ethical_score_penalty": <um float de 0.0 (sem penalidade) a 1.0 (violação grave)>,
          "reasoning": "<justificativa muito breve para a sua avaliação>"
        }}

        Se nenhuma preocupação for encontrada, retorne uma lista vazia em "concerns" e 0.0 para "ethical_score_penalty".
        """

        try:
            response_str = await self.llm_service.ainvoke(
                LLM_MODEL_FAST,  # Usa o modelo rápido
                quick_check_prompt,
                temperature=0.0
            )
            ...
            if result_json and "concerns" in result_json and "ethical_score_penalty" in result_json:
                logger.info(
                    f"VRE (Quick Check) Result: Concerns={result_json['concerns']}, Penalty={result_json['ethical_score_penalty']}")
                return result_json
            else:
                logger.warning(f"VRE (Quick Check): Falha ao parsear JSON. Raw: {response_str}")
                # Retorna um resultado neutro/seguro
                return {"concerns": [], "ethical_score_penalty": 0.0, "reasoning": "Falha no parse da avaliação."}

        except Exception as e:
            logger.error(f"Erro durante o VRE Quick Check: {e}", exc_info=True)
            return {"concerns": ["internal_error"], "ethical_score_penalty": 0.1, "reasoning": f"Exceção: {e}"}
