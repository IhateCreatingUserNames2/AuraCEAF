...
# FASE 1: Configuração Centralizada (inspirado na Claude)
DEFAULT_DRIVE_CONFIG = {
    # Taxas de mudança passiva (por hora)
    "passive_decay_rate": 0.03,
    "passive_curiosity_increase": 0.05,
    "passive_connection_increase": 0.02,

    # Reação a eventos do turno
    "mastery_satisfaction_on_success": 0.4,
    "mastery_boost_on_prediction_error": 0.5,  # Aumento forte por "surpresa"
    "consistency_boost_on_failure": 0.15,
    "consistency_boost_on_success": 0.10,  # Reforça o que funciona
    "curiosity_satisfaction_on_topic_shift": 0.15,
    "curiosity_boost_on_low_memory": 0.06,

    # Parâmetros de Meta-Aprendizado
...
class MotivationalEngine:
    ...
    def __init__(self, config: Dict = None):
        self.config = config or DEFAULT_DRIVE_CONFIG
        self.performance_history: deque[Dict] = deque(maxlen=self.config["performance_history_window"])
        logger.info("MotivationalEngine (V3 Synthesis) inicializado.")
    ...
    def _apply_temporal_decay(self, drives: MotivationalDrives):
        """Aplica decaimento e crescimento natural dos drives com o tempo."""
        cfg = self.config
        time_delta_hours = (time.time() - drives.last_updated) / 3600

        decay_factor = cfg['passive_decay_rate'] * time_delta_hours
        ...
        drives.curiosity.intensity += cfg['passive_curiosity_increase'] * time_delta_hours
        drives.connection.intensity += cfg['passive_connection_increase'] * time_delta_hours
    ...
    def _apply_feedback(self, drives: MotivationalDrives, metrics: dict):
        ...
        if vre_rejections == 0 and final_confidence > 0.75:
            ...
            boost = cfg['consistency_boost_on_success'] * drives.consistency.effectiveness_score
            drives.consistency.intensity += boost
            logger.info(
                f"MOTIVATION (Success): Mastery satisfeito (-{satisfaction:.2f}), Consistency reforçado (+{boost:.2f})")
        ...
        if prediction_error > 0.2:
            boost = cfg['mastery_boost_on_prediction_error'] * prediction_error * drives.mastery.effectiveness_score
        ...
        if metrics.get("relevant_memories_count", 5) < 2:
            boost = cfg['curiosity_boost_on_low_memory'] * drives.curiosity.effectiveness_score
    ...
    def _update_momentum(self, new_drives: MotivationalDrives, old_drives: MotivationalDrives):
        ...
        for drive_name in ["curiosity", "connection", "mastery", "consistency"]:
            ...
            new_drive.momentum = (old_drive.momentum * self.config['momentum_decay']) + change
    ...
    def _meta_learn(self, current_drives: MotivationalDrives):
        ...
        for drive_name in ["curiosity", "connection", "mastery", "consistency"]:
            ...
            adjustment = self.config['meta_learning_rate'] * effectiveness_signal
