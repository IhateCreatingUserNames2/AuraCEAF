...
class AgentRepository:
    ...
    def get_recently_active_agent_ids(self, hours: int = 48) -> list[str]:
        """
        Retorna uma lista de IDs de agentes que tiveram atividade em sessões
        de chat nas últimas X horas.
        """
        with self.SessionLocal() as session:
            cutoff_time = datetime.utcnow() - timedelta(hours=hours)

            # Query para encontrar agent_ids distintos de sessões ativas recentes
            results = session.query(ChatSession.agent_id).filter(
                ChatSession.last_active >= cutoff_time
            ).distinct().all()

            # Extrai os IDs da tupla de resultados
            agent_ids = [row[0] for row in results]
            return agent_ids