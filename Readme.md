...
Such Systems if feed unsorted data should output equally uniqueness. The large language model dictates the quality of the data translated. It would, In a better scenario, be truly unique, when the large language model could be trained in the Output data from the system, throught it's various data registers. The use of "Glyphs", Unique Identification ID Keys within the diemensions which shall resonante in the lantent space, would allow modulation of the output would be more precise with the intended objective. 
