...
class LLMConfig(BaseModel):
    ...
    fast_model: str = Field("openrouter/x-ai/grok-4-fast",
                            description="Modelo para tarefas rápidas e rotineiras.")
    smart_model: str = Field("openrouter/x-ai/grok-4-fast",
                             description="Modelo para raciocínio complexo e síntese.")
    creative_model: str = Field("openrouter/x-ai/grok-4-fast",
    ...
    # Timeout settings
    timeout_seconds: float = Field(120.0, description="Tempo limite para chamadas de API.")
...
class DrivesConfig(BaseModel):
    ...
    passive_curiosity_increase: float = Field(0.05, description="Crescimento passivo da curiosidade por hora.") # Renomeado para bater com motivational_engine.py
    passive_connection_increase: float = Field(0.08, description="Crescimento passivo da necessidade de conexão por hora.") # Renomeado

    # Reação a eventos (Novos campos para controle fino)
    mastery_satisfaction_on_success: float = Field(0.4, description="Queda da maestria após sucesso (satisfeito).")
    consistency_boost_on_failure: float = Field(0.15, description="Aumento da necessidade de consistência após falha (ficar conservador).")
