...
class MotivationalEngine:
    ...
    def _apply_temporal_decay(self, drives: MotivationalDrives):
        ...
        # Acessa via ponto (.) em vez de chave ['']
    ...
    def _apply_feedback(self, drives: MotivationalDrives, metrics: dict):
        ...
        # Lógica de Falha
        if vre_rejections > 0:
            boost = self.config.consistency_boost_on_failure * drives.consistency.effectiveness_score
        ...
        if vre_rejections == 0 and final_confidence > 0.75:
            ...
            satisfaction = self.config.mastery_satisfaction_on_success
            drives.mastery.intensity -= satisfaction
            logger.info(
        ...
        if metrics.get("topic_shifted_this_turn", False):
            drives.curiosity.intensity -= self.config.curiosity_satisfaction_on_topic_shift
